...
function TournamentCard({ tournament }) {
  ...
  const handleAddParticipant = (participant) => {
    ...
    setShowForm(false); // الفورم يختفي بعد التسجيل
  };

  const handleRegisterClick = () => {
    if (!isRegistered) {
      // إذا ما كنتش مسجلة، نوري الفورم
      setShowForm(true);
    } else {
      // إذا مسجلة من قبل، نقلب الحالة ونخلي الفورم مخفي
      setIsRegistered(false);
      setShowForm(false);
      // ممكن تحذفي آخر مشارك مسجل إلا بغيتِ
